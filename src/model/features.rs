...
use crate::{
    binance::models::{fapi_exchange_info::Symbol, orderbook::OrderBooksRWL, trades::Trade},
    MIN_TICKS_FOR_SIGNAL, MIN_TRADES_TO_START,
};
/*
feature indexes
0: trade time
...
pub async fn manage_model(
    ...
    loop {
        ...
        for i in 0..feature_vec.len() - 1 {
            if i >= rolling_period {
                let tick_difference = (feature_vec[i][8] - feature_vec[i - rolling_period][8])
                    / tick_size.to_f32().unwrap();
        ...
        //except for the model, the dmats should be stored offsite in S3. the model can stay in the container
        let timestamp = chrono::Utc::now().timestamp();
        let name = format!("{}-{}-xgb.model", market.symbol, timestamp);
        bst.save(&name).unwrap();
        dtest
            .save(&format!("{}-{}-xgb.dmat", market.symbol, timestamp))
            .unwrap();
        //upload dmat object to ECS and delete from here
    }
}
